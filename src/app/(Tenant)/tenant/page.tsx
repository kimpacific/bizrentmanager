...
export default function TenantPortal({ onLogout, userProfile }: { onLogout: () => void; userProfile: UserProfile }) {
  const [showSettings, setShowSettings] = useState(false)
  const [activeTab, setActiveTab] = useState("dashboard")

  const navigationItems = [
    { id: "dashboard", label: "Dashboard", icon: LayoutDashboard, description: "Overview & analytics" },
    { id: "profile", label: "My Profile", icon: UserIcon, description: "Personal information" },
    { id: "invoices", label: "Invoices", icon: FileText, description: "View all invoices" },
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 transition-all duration-700 ease-in-out">
            ...
            <div className="mb-8 p-4 bg-gradient-to-r from-orange-50 to-orange-100/50 rounded-xl border border-orange-200/50">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-600">{userProfile?.name || "Tenant"}</p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-8 p-4 bg-gradient-to-r from-slate-50 to-slate-100/50 rounded-xl border border-slate-200/50">
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-slate-800">$1,200</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-600">Pending</span>
                  <span className="font-medium text-orange-600">$300</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="p-8">
            <div className="bg-white/60 backdrop-blur-sm rounded-2xl shadow-lg border border-slate-200/60 min-h-[calc(100vh-8rem)]">
              <div className="p-6">
                {activeTab === "dashboard" && <TenantDashboard userProfile={userProfile} />}
                {activeTab === "profile" && <TenantProfile userProfile={userProfile} />}
                {activeTab === "invoices" && <TenantInvoices userProfile={userProfile} />}
                {activeTab === "payments" && <TenantPayments userProfile={userProfile} />}
              </div>
            </div>
          </div>
        </div>
      ...
      {showSettings && <SettingsModal onClose={() => setShowSettings(false)} userType="tenant" />}
    </div>
  )
}
